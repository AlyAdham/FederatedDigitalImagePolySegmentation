...
class PolyGen(Dataset):
    def __init__(self, root='PolypGen2021_MultiCenterData_v3/', center = 1, transform=None, show_name=False):
        self.data = []
        self.label = []
        self.name = []
        ...
        for c in center:
            ...
            for f in files:
                ...
                # Handle special characters in filenames - remove brackets and other special chars
                base_name = f.replace('.jpg', '')
                # Remove brackets and special characters from base name
                clean_name = base_name.replace(']', '').replace('[', '').replace(')', '').replace('(', '')
                mask_name = clean_name + '_mask.jpg'
                self.label.append(os.path.join(os.path.join(root, 'data_C'+str(c), 'masks_C'+str(c), mask_name)))
                self.name.append(f)
